...
class Train 
  ...
  def check_occupied_place
    occupied_place = @wagons.sum {|w| w.occupied_place}
    capacity = @wagons.sum {|w| w.capacity}

    return true if capacity - occupied_place > 0 

    puts "В этом поезде нет свободного места, Использовано #{occupied_place}, всего #{capacity}" 
    false
  end
